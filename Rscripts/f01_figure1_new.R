...
p0 <- vector("list",length(fl))
p1 <- vector("list",length(fl))
p2 <- vector("list",length(fl))
p3 <- vector("list",length(fl))
for(i in seq_along(countries)){
  res <- readRDS(paste0("processed_data/tracking/figure1/",countries_short[i],".RDS"))
  dat1 <- res[[1]]
  dat2 <- res[[2]]
  dat3 <- res[[3]]
  # p0[[i]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[i],angle=90),
  #                              family="serif",size=12,fontface="bold")+theme_void()
  p1[[i]] <- ggplot(dat1,aes(x="",y=value,fill=type))+ 
  ...
  p3[[i]] <- ggplot(dat3,aes(x="",y=value,fill=type))+ geom_bar(stat="identity", width=1) +
    ...
    theme_void()+
    labs(title = ifelse(i==1,"Visits of <span style='color:#AA8939'>political</span> and <span style='color:#303C74'>non-political</span><br> news articles",""))  +
    theme(legend.position = "none",
          text = element_text(family = "serif",size=16),
          plot.title.position = "plot",
...
p0[[1]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[1],angle=90),
                              family="serif",size=12,fontface="bold")+theme_void()
p0[[2]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[2],angle=90),
                              family="serif",size=12,fontface="bold")+theme_void()
p0[[3]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[3],angle=90),
                              family="serif",size=12,fontface="bold")+theme_void()
p0[[4]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[4],angle=90),
                              family="serif",size=12,fontface="bold")+theme_void()
p0[[5]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[5],angle=90),
                              family="serif",size=12,fontface="bold")+theme_void()
p0[[6]] <- ggplot()+geom_text(aes(x=0,y=0,label=countries[6],angle=90),
                              family="serif",size=12,fontface="bold")+theme_void()

p <- c(p0,p1,p2,p3)
pfull <- wrap_plots(p,ncol = 4,nrow = length(fl),byrow = FALSE)+plot_layout(widths = c(0.1,0.3,0.3,0.3))
pfull
ggsave("figures/figure1_new.pdf",pfull,width = 15,height=25)
