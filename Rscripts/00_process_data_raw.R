...
## mobile ----
countries <- c("ESP","FRA","GER","ITA","UK","USA")
fl <- paste0("data/tracking/data_raw/",countries,"/2019-06-16_all_mobile.rda")
out_files <- c("es.csv","fr.csv","de.csv",
               "it.csv","uk.csv" ,"us.csv")

#load auxiliary data
domain_lists <- readRDS("data/domain_lists_mobile.rds")
doms <- paste(domain_lists$newsportals, collapse = "|")
political_urls <- as.data.table(readRDS("data/political_URLs.rds"))
news_types <- as.data.table(readRDS("data/news_types.rds"))
...
for (i in 1:6) {
  ...
  if (!dir.exists("processed_data")) dir.create("processed_data")
  if (!dir.exists("processed_data/tracking")) dir.create("processed_data/tracking")
  if (!dir.exists("processed_data/tracking/mobile")) dir.create("processed_data/tracking/mobile")
  
  fwrite(dt1, paste0("processed_data/tracking/mobile/", out_files[i]))
}

if (!dir.exists("processed_data/tracking/mobile/news_only")) dir.create("processed_data/tracking/mobile/news_only")

fl <- list.files("processed_data/tracking/mobile",full.names = TRUE,pattern = "csv")
...
for(i in seq_along(fl)){
  ...
  dt[,prev_type:=fifelse(shift(duration,n = 1L, fill = 5000)>3600,"direct",prev_type)]
  dtnews <- dt[type=="news"]
  fwrite(dtnews,stringr::str_replace(fl[i],"mobile","mobile/news_only"))
}
