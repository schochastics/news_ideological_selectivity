...
news <- map_dfr(cutoffs,function(y) { lapply(fl,function(x){
  ...
  tibble(country = x,cutoff = y,
         popsd = mean(dt[,.(avg_ideo_aud=psd(avg_ideo)),.(panelist_id)][["avg_ideo_aud"]],na.rm=TRUE), 
         type="all news")
...
news <- map_dfr(cutoffs,function(y) { lapply(fl,function(x){
  ...
  tibble(country = x,cutoff = y,
         popsd = mean(dt1[,.N,.(panelist_id,lcr)][,.(frac=N*(N-1)/(sum(N)*(sum(N)-1))),.(panelist_id)][,.(Diversity=1-sum(frac^2,na.rm = TRUE)),.(panelist_id)][["Diversity"]]), 
...
ggsave("figures/old/explore/avg_diversity_scores.png",width = 10,height=6,bg = "white")
